...
public:
//DMT
//     Using uint for indices is not a good idea. uint, unlike int, has well defined overflow behaviour,
//     preventing compilers from taking shortcuts and having to issue code for handling all cases.
...
//DMT linear access, ref to int (indices) adds only overhead so i'm using copies instead
    inline data_type& access(const index_type i) {
#ifdef DEBUG
        if ( i >= wr*wc ) ERROR("Element index out of bounds")
#endif
        return data[i];
    }
    inline const data_type& access(const index_type i) const {
#ifdef DEBUG
        if ( i >= wr*wc ) ERROR("Element index out of bounds")
#endif
        return data[i];
...
//DMT
// Refereces word sized (as pointers). On x86-64 chances are int is is shorter than & or * so.
// Since we don't really need a reference there is no point in passing it.
    FMatrix(const index_type r, const index_type c, const data_type *v) : wc(c), wr(r), data(v, r*c) {}
    ...
    template <class T> FMatrix(const PCrsMatrix<T>& s);
