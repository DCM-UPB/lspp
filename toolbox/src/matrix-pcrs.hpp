...
public:
   ...
   template<class T> PCrsMatrix(const   FMatrix<T>& s, const MPI_Comm ncomm=MPI_COMM_WORLD);
...
//NOTE: this is potentially dangerous, shalt remember to match it with whatever index_type is. mpi_index isn't great either as it is cast into a number of chars //DMT
       MPI_Allreduce(&lsize, &gsize, 1, MPI_UNSIGNED_LONG, MPI_SUM, mycomm); // currently index_type is unsigned int, not long, it is "lucky" on x86-64 they are of the same size. //DMT
       return gsize;
    }
